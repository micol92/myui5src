sap.ui.define([
    ...
    return BaseController.extend("hkmc.poc.ns.ui5demo.controller.CreateTODiv", {
        ...
        _onRouteMatchedEduBizCreate: function(event){
           ...
           // bizData.push(bizData2);
         /*     
            if(this.getView().getModel("newBizLayout")){
                debugger;
                this.getView().getModel("newBizLayout").setData(bizData);
            }else{
                debugger;
                this.getView().setModel(new JSONModel(bizData), "newBizLayout");
            };
          */  
            //debugger;
            //Create Model
            ...
            //기존 데이터 복사 생성
        /*    
            var copyModel = this.getOwnerComponent().getModel("create");
            if(copyModel && Object.keys(copyModel.getData()).length){
                this.getView().getModel("newEduBiz").setData(JSON.parse(JSON.stringify(copyModel.getData())));
            }else{  //신규 생성
                this.getView().getModel("newEduBiz").setData({});
            }
            this.getView().byId("fileUploader").setValue(null); //기존 데이터 중 파일만 제외
        */
        },
